...
public class EasyExplorePlugin extends AbstractUIPlugin {
	...
	public static void log(Object msg) {
		...
		Status status = new Status(1, getDefault().getDescriptor()
				.getUniqueIdentifier(), 4, msg + "\n", null);
		log.log(status);
	}
	...
	public static void log(Throwable ex) {
		...
		Status status = new Status(4, getDefault().getDescriptor()
				.getUniqueIdentifier(), 4, msg, null);
		log.log(status);
	}
	...
	protected void initializeDefaultPreferences(IPreferenceStore store) {
		...
		if (osName.indexOf("Windows") != -1) {
			...
			String distritor = LinuxSystemUtils.getLinuxOSDistributionName();
			if("ubuntu".equalsIgnoreCase(distritor)){
				defaultTarget = "nautilus --browser {0}";
			}else{
				defaultTarget = "xdg-open {0}";
			}
		}
	...
	public boolean isSupported() {
		...
		return ((osName.indexOf("Windows") != -1)
				|| (osName.indexOf("Mac") != -1) || (osName.indexOf("Linux") != -1));
		/*     */}
	/*     */
}
